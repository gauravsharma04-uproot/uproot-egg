...
DB_PATH = os.path.join(os.path.dirname(__file__), "egg.db")
...
def init_db():
    ...
    if not existing:
        cur.execute("INSERT INTO settings(key, value) VALUES(?, ?)", ("config", json.dumps(DEFAULT_SETTINGS)))
        conn.commit()
    conn.close()
...
def reveal():
    if not session.get("started"):
        return redirect(url_for("index"))
    # enforce one reveal per session
...
def dashboard():
    ...
    return render_template("admin.html", claims=claims, settings=settings, total=total, redeemed=redeemed)
...
def redeem(code):
    ...
    if row["redeemed"]:
        ...
        return redirect(url_for("dashboard"))
    conn.execute("UPDATE claims SET redeemed=1, redeemed_at=?, redeemed_by=? WHERE code=?",
